...
wchar_t handleMultiByteChar(Iterator& in, const Iterator& end) {
  ...
  assert(*in &
         0xC0);  // Must have top two bits set to use this func or all will fail

  /* Using the chart here: https://en.wikipedia.org/wiki/UTF-8#Description
   * ===========================================================================================================
   * Bits of    | First      | Last Code  | Byte 1      | Byte 2   | Byte 3   |
   ...
   *|          |          |
   * 21         | U+10000    | U+1FFFFF   | 4  11110xxx | 10xxxxxx | 10xxxxxx |
   *10xxxxxx |          |          |
   * 26         | U+200000   | U+3FFFFFF  | 5  111110xx | 10xxxxxx | 10xxxxxx |
   *10xxxxxx | 10xxxxxx |          |
   * 31         | U+4000000  | U+7FFFFFFF | 6  1111110x | 10xxxxxx | 10xxxxxx |
   *10xxxxxx | 10xxxxxx | 10xxxxxx |
   *
   * If the 1st byte matches the 110x xxxx pattern (0xC0) .. we know there is
   *one more byte and so on
   **/

  // Work out how many digits to go by shifting the mask left until we find a
  // match
  while ((*in & mask) != mask) {
    --digitsToGo;
    mask <<= 1;
