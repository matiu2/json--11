...
#include "parser.hpp" // Just for StringInfo
...
struct JSON {
public:
  enum Type { null, boolean, number, text, string_info, map, list };
...
private:
  ...
  union Value {
    ...
    Value(StringInfo val) : as_string_info(val) {}
  ...
  void copyFromOther(const JSON &other) {
    ...
    case text:
      ...
      break;
    case sring_info:
      value.as_string_info = other.value.as_string_info;
      break;
    case map:
  ...
  /// Finally reads the lazy string
  std::string& manifestString() {
    assert(type == string_info);
    std::string newVal = value.as_string_info;
    new (&value.as_string) std::string(std::move(newVal));
    type = text;
...
public:
  ...
  operator std::wstring() const {
    assert(type == text);
    std::wstring result;
    const std::string& input = manifestString();
    result.reserve(input.size() * 1.10); // Assume a 10% size increase
    utf82wchar(input.cbegin(), input.cend(),
  ...
  explicit operator bool() const {
    ...
    case string_info:
      return value.as_string_info.begin() != value.as_string_info.end();
    case map:
  ...
  bool operator==(const JSON &other) const {
    if (other.type == type)
      ...
      case string_info:
        return value.as_string_info == other.value.as_string_info;
      case map:
...
std::ostream &operator<<(ostream &s, const JSON &j) {
  ...
  case JSON::text:
    ...
    break;
  case JSON::string_info:
    s << '"' << utf82json(j.manifestString()) << '"';
    break;
  case JSON::map: {
    s << j.value.as_map;
    break;
...
JSON JBool(bool val) { return JSON(val, 1); }

template <typename T> std::vector<T> jsonToHomogenousList(const JSON &j) {
  std::vector<T> result;
  const JList &input = j;
